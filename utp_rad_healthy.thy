...
(* Paper Theorem 9. *)
lemma RA1_A0_absorb: "RA1 (A0 P) = RA1 P"
...
lemma II_Rac_is_RA1 [closure]:
  "II_Rac is RA1"
  by (rule Healthy_intro, rule RA1_II_Rac)

lemma II_Rac_is_RA2 [closure]:
  "II_Rac is RA2"
...
lemma II_Rac_is_PBMH_ades [closure]:
  "II_Rac is PBMH_ades"
...
lemma rad_wait_false_true:
  "(true \<^sub>wf) = true"
  by (simp add: rad_wait_false_def fun_eq_iff subst_app_def subst_upd_def
      subst_id_def SEXP_def; pred_auto)

lemma rad_wait_false_false:
  "(false \<^sub>wf) = false"
  by (simp add: rad_wait_false_def fun_eq_iff subst_app_def subst_upd_def
      subst_id_def SEXP_def; pred_auto)

lemma rad_wait_false_ac_non_empty:
  "(ac_non_empty \<^sub>wf) = ac_non_empty"
...
proof -
  ...
  also have "... = (RA \<circ> A) (P \<squnion> Q)"
    by (rule RA_A_absorb[OF choice_PBMH choice_design, symmetric])
  finally show ?thesis .
qed
...
lemmas RA_design_wait_false_ok_true' =
...
proof -
  ...
  also have "... = RA2 (PBMH_ades (RA1 (PBMH_ades P)))"
    by (simp only:
        PBMH_ades_RA2_absorb[simplified comp_apply])
...
proof -
  show ?thesis
    ...
    apply (simp only: RA_design_wait_false'
        RA_design_wait_false_ok_true')
